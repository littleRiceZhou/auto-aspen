...
def _build_agent(model_name: str) -> Agent:
    instructions = (
        ...
        or (
            ...
            "3. **流程图展示**：仿真成功后，必须使用 Markdown 图片语法展示流程图，格式为：`![流程图](图片路径)`。请务必使用工具返回的 `diagram_url`，确保以 `/static` 开头，且必须闭合括号 `)`。\n"
            "4. **文件下载展示**：仿真结果中的技术报告（在 `document_urls` 中），必须使用链接语法展示，格式为：`[点击下载：技术报告名称](文件路径)`。确保路径以 `/static` 开头。\n"
            "5. **回复完整性**：请确保你的回复是完整的 Markdown 格式，不要在输出路径时突然中断。"
        )
    )
    return Agent(
        name="Assistant",
